...
func (ssocket *SSocketWrapper) Copy2RaW(raw net.Conn) (err error) {
	...
	if err != nil || n <= 0 {
		utils.Logger.Fatal("parse int from socket failed! ", err,  "bytes: ", n)
		return err
	}
	...
	if err != nil || cn <= 0 {
		utils.Logger.Fatal("parse content from socket failed! ", err,  "bytes: ", cn)
		return err
	}

	out, err := sercurity.Uncompress(content, ssocket.iv, sercurity.MakeCompressKey(ssocket.cipher))
	if err != nil {
		utils.Logger.Fatal("content decrypt failed! ", err)
		return err
	}

	on, err := raw.Write(out)
	if err != nil {
		utils.Logger.Fatal("write content to raw failed! ", err, "write bytes: ", on)
		return err
	}
...
func (ssocket *SSocketWrapper) WriteFromRaw(raw net.Conn) (err error){
	...
	rn, err := raw.Read(buf)
	if err != nil || rn <= 0 {
		utils.Logger.Fatal("read from raw socket ", err)
		return err
	}

	out, err := sercurity.Compress(buf, ssocket.iv, sercurity.MakeCompressKey(ssocket.cipher))
	if err != nil {
		utils.Logger.Fatal("content encrypt failed! ", err)
		return err
	}

	on, err := ssocket.src_socket.Write(utils.Int2byte((uint32)(len(out))))
	if err != nil {
		utils.Logger.Fatal("write content to SSocket failed! ", err, "write bytes: ", on, "bytes.")
		return err
	}

	n, err := ssocket.src_socket.Write(out)
	if err != nil {
		utils.Logger.Fatal("write content to SSocket failed! ", err, "write bytes: ", n, "bytes.")
		return err
	}
